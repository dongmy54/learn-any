---
name: webextract
description: 把任意网页提取为 Markdown/JSON（单页或 crawl 批量抓取整站），保留表格/代码/列表等结构、剥离导航广告，支持静态页与 SPA 动态页。当用户给出 URL 想了解/总结/翻译/引用网页，或要批量下载整站、文档站时使用。
---

webextract 将网页正文提取为 Markdown（或 JSON），优先保证「人在页面上能看到的内容」完整不丢失——表格、代码块、列表等结构化内容按原顺序保留，同时剥离导航/广告/侧边栏等噪声。详见各【工作流程】步骤。
...
1. **默认单页**：只给 URL 一律走单页；仅在用户明确表达批量/整站/全部下载意图时才用 `crawl`，意图模糊也按单页处理。
2. **完整优先**：保证页面可见正文（含表格/代码/列表）完整且保序，宁可多留也不要干净但残缺。
3. **静态优先，动态兜底**：先不加 `--render`（快，约 1 秒）；仅当输出明显残缺/过短、或已知是 SPA 时才加 `--render` 重抓。
...
| 信号 | 模式 |
|------|------|
| 一个 URL，想了解/总结/翻译/引用**这一页**；读在线文档/API 文档/技术文章作为上下文 | 单页（默认） |
...
1. **渲染**：先不加 `--render` 抓取；输出完整即直接用，明显残缺/过短/已知 SPA 则加 `--render` 重抓。
2. **格式**：需要结构化字段（标题、摘要、源 URL）时加 `--format json`。

## 输出

- **单页**：默认输出到 stdout；`-o` 写文件；`--format json` 输出 `title/description/url/content` 四字段。
- **批量**：默认写入当前项目下 `docs` 目录（可 `-o` 覆盖）。
  - 种子页固定落盘 `<output>/<host>/index.md`，**不受 `--max` 截断影响**。
  - 其余页落盘 `<output>/<host>/<path-slug>.md`。
  - 每个文件首行带标题与源 URL，便于溯源。
...
## 附录：工具构建（仅当平台无预编译二进制时）

`scripts/` 下已附带 `webextract-{darwin,linux}-{arm64,amd64}`，包装脚本自动选用，无匹配时自动回退源码构建——正常使用无需手动编译。仅当平台不在其中（如 Windows）或需重新构建时才手动操作（需 Go 1.26+）：
